...
export default function EditExpenseForm({ group, expense, onClose, onExpenseUpdated }) {
  const { user, token } = useAuth();
  const [description, setDescription] = useState(expense.description);
  const [amount, setAmount] = useState(expense.amount);
  const [paidBy, setPaidBy] = useState(expense.paid_by_user_id || user?.id);
