    ...
    return (
        <div className="bg-white dark:bg-black min-h-screen flex items-center justify-center px-4">
            <div className="max-w-md w-full text-center">
                {status === 'verifying' && (
                    <h1 className="text-3xl font-black text-black dark:text-white uppercase animate-pulse">Vérification
                        en cours...</h1>
                )}
                {status === 'success' && (
                    <div className="bg-green-50 p-8 border-4 border-green-500">
                        ...
                        <p className="mt-4 text-lg text-green-800">Votre compte est maintenant actif. Vous allez être
                            redirigé vers la page de connexion.</p>
                        <Link href="/login"
                              className="mt-6 inline-block font-bold text-green-600 hover:underline">Connexion</Link>
                    </div>
                )}
                {status === 'error' && (
                    <div className="bg-red-50 p-8 border-4 border-red-500">
                        ...
                        <Link href="/register" className="mt-6 inline-block font-bold text-red-600 hover:underline">S&#39;inscrire
                            à nouveau</Link>
                    </div>
                )}
            </div>
